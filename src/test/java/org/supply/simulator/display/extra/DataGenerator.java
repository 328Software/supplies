...
import org.supply.simulator.display.mock.data.*;
import org.supply.simulator.display.renderable.menu.impl.BasicMenuRenderable;
...
public class DataGenerator {
    ...
    private HashMap<String,TexturedEntityType> typeMap;
    private HashMap<String,MockTextureType> textureMap;
    public DataGenerator () {
        typeMap = new HashMap<>();
        textureMap = new HashMap<>();
    }

    public MockChunk createChunk(int chunkRows,int chunkColumns, int offsetX, int offsetY) {
        //TODO store chunktype in map too?
        MockChunkType type = new MockChunkType();
    ...
    public BasicUnitRenderable createUnit(float topLeftX, float topLeftY, float topLeftZ, float length, float width, String name) {
        BasicUnitRenderable renderable = new BasicUnitRenderable();
        MockUnit unit = new MockUnit();
        unit.setUnitPositions(getUnitPositions(topLeftX, topLeftY, topLeftZ,  length,  width));

        MockUnitType type;
        if (typeMap.containsKey(name)) {
            type = (MockUnitType)typeMap.get(name);
        } else {
            type = new MockUnitType();
            TextureData data = getTextureData(name);
            type.setTextureType(data.textureType);
            type.setSubInfo(data.subInfo);
            typeMap.put(name,type);
        }

        unit.setType(type);
        renderable.setEntity(unit);
        return renderable;
    }

    public BasicUnitRenderable createUnit(float topLeftX, float topLeftY, float topLeftZ, float length, float width) {
        return createUnit( topLeftX, topLeftY, topLeftZ,  length,  width,"textures/alexsface.png");
    }

    public BasicMenuRenderable createMenu(float topLeftX,float topLeftY,float topLeftZ, float length, float width, String name) {
        BasicMenuRenderable renderable = new BasicMenuRenderable();
        MockMenu menu = new MockMenu();
        menu.setPositions(getUnitPositions(topLeftX, topLeftY, topLeftZ, length, width));
        MockMenuType type;

        if (typeMap.containsKey(name)) {
            type = (MockMenuType)typeMap.get(name);
        } else {
            type = new MockMenuType();
            TextureData data = getTextureData(name);
            type.setTextureType(data.textureType);
            type.setSubInfo(data.subInfo);
            typeMap.put(name,type);
        ...
        menu.setType(type);
        renderable.setEntity(menu);
        return renderable;
    }

    private MockUnitPositions getUnitPositions(float topLeftX,float topLeftY,float topLeftZ, float length, float width) {
        ...
        MockUnitPositions entityData = new MockUnitPositions();
